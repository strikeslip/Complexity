<!--
[SOS - Sounds Of Seismic] - SHADOWZONE Edition
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .layer-toggle.texture {
            border-color: var(--texture-color);
            color: var(--texture-color);
        }
        .layer-toggle.texture.active {
...
<body>
    <div class="container">
        ...
        <div class="controls-panel" id="controlsPanel">
            ...
            <div class="control-group">
                ...
                <span class="value" id="stretchValue">1.0×</span>
            </div>
            <div class="control-group">
                <label>Reverb</label>
        ...
        <div class="metrics">
            <div class="metric">
                <div class="metric-value texture" id="textureValue">—</div>
                <div class="metric-label">Texture</div>
            </div>
            <div class="metric">
                <div class="metric-value tone" id="toneValue">—</div>
                <div class="metric-label">Tone</div>
            </div>
            <div class="metric">
                <div class="metric-value rhythm" id="rhythmValue">—</div>
                <div class="metric-label">Rhythm</div>
            </div>
            <div class="metric">
                <div class="metric-value" id="filterValue">—</div>
    ...
    class ShadowZoneVisualizer {
        ...
        update(amplitude, playhead) {
            this.seismicAmplitude = clamp(amplitude, 0, 1);
            this.playhead = playhead;
            this.wavefrontPhase = (playhead * 2.5) % 1;
        }
        ...
        drawWavefronts() {
            ...
            for (let w = 0; w < 3; w++) {
                const phase = (this.wavefrontPhase + w / 3) % 1;
                const alpha = 1 - w * 0.3;
        ...
        drawEpicenter() {
            ...
            // Pulsing glow
            const pulse = 12 + this.seismicAmplitude * 25 + Math.sin(this.wavefrontPhase * Math.PI * 4) * 5;
        ...
        drawLabels() {
            ...
            ctx.fillText('EPICENTER', ex, ey - 22);
            
            // Station label
            const sx = this.cx + this.R * Math.cos(this.stationAngle);
            const sy = this.cy + this.R * Math.sin(this.stationAngle);
            ctx.fillStyle = '#4aff9e';
            ctx.fillText('IU.ANMO', sx + 18, sy + 20);
            ...
            for (const side of [1, -1]) {
                ...
                const lx = this.cx + this.R * 1.15 * Math.cos(labelAngle);
                const ly = this.cy + this.R * 1.15 * Math.sin(labelAngle);
            ...
            ctx.font = '8px monospace';
            ctx.textAlign = 'center';
            ctx.fillText('MANTLE', this.cx, this.cy - this.R * 0.75);
            
            ctx.fillStyle = 'rgba(255, 200, 150, 0.45)';
            ctx.fillText('OUTER CORE', this.cx, this.cy - this.R * 0.35);
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // SYNTH GRAIN - Oscillator-based grain with envelope
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // GRANULAR CLOUD - Generates streams of grains
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    class GranularCloud {
        ...
        scheduleGrain() {
            ...
            // Calculate frequency with spread
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // RHYTHM CLOUD - Zero-crossing triggered grains
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // GRANULAR ENGINE - Manages three cloud layers
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    class GranularEngine {
        ...
        initializeClouds(rootFreq, scale) {
            ...
            // TONE: Sparse, long, fundamental - harmonic foundation
        ...
        setRootFreq(f) {
            if (this.textureCloud) {
                this.textureCloud.rootFreq = f;
                this.textureCloud.baseFrequency = f * 4;
            }
            if (this.toneCloud) {
                this.toneCloud.rootFreq = f;
                this.toneCloud.baseFrequency = f;
            }
            if (this.rhythmCloud) {
                this.rhythmCloud.rootFreq = f;
                this.rhythmCloud.baseFrequency = f * 2;
            }
        }
    ...
    let reverb, reverbWet;
    let delay, delayFeedback;
    let seismicData = [];
    let isPlaying = false;
    let isFrozen = false;
    let quakeInfo = null;
    ...
    function parseMiniSEED(buffer) {
        try {
            ...
            // Fallback seismic-like waveform
    ...
    function updateVisualization() {
        ...
        // Update shadow zone
        shadowViz?.update(amp, playhead);
        shadowViz?.draw();
        
        // Update metrics
        if (granularEngine) {
            const stats = granularEngine.getStats();
            textureEl.textContent = granularEngine.textureCloud?.isActive ? Math.round(granularEngine.textureCloud.density) : '—';
            toneEl.textContent = granularEngine.toneCloud?.isActive ? Math.round(granularEngine.toneCloud.density) : '—';
            rhythmEl.textContent = granularEngine.rhythmCloud?.isActive ? Math.round(granularEngine.rhythmCloud.triggerProbability * 100) + '%' : '—';
    ...
    stretchSlider.oninput = function() {
        document.getElementById('stretchValue').textContent = parseFloat(this.value).toFixed(2) + '×';
    ...
    reverbSlider.oninput = function() {
        ...
        if (reverbWet && audioCtx) {
            reverbWet.gain.setTargetAtTime(parseInt(this.value) / 100 * 0.7, audioCtx.currentTime, 0.1);
        }
    };
    ...
    console.log('[SOS] SHADOWZONE Edition loaded');
    </script>
</body>
</html>